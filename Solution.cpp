...
Solution::Solution(Solution solution){
    m = solution.m;
    k =solution.k;
    conflictedge = solution.conflictedge;
    conflictPointNumber = solution.conflictPointNumber;
    iter = solution.iter;
    memcpy(adjColorList,solution.adjColorList, sizeof(adjColorList));
    memcpy(tabulist,solution.tabulist, sizeof(tabulist));
    memcpy(sol,solution.sol, sizeof(sol));
    memcpy(conflictpoint,solution.conflictpoint, sizeof(sol));
    memcpy(conflictPointPosition,solution.conflictPointPosition, sizeof(conflictPointPosition));
    adjVertexListin = solution.adjVertexListin;
}
Solution:: Solution(AdjVertexList adjVertexList,int colornumber,int sol1[1000]){
    adjVertexListin = adjVertexList;
    m = adjVertexList.size();
    k = colornumber;
    memcpy(sol,sol1, sizeof(sol1));
    //初始化邻接颜色表
    for(int i= 0;i < m;i++){
        for (int j =0;j < adjVertexList[i].size() ; ++j) {
            int color = sol[adjVertexList[i][j]];
...
void Solution::tabusearch(int Maxiter){
    ...
    while(conflictedge >0 && iter < Maxiter) {
        ...
        cout<<iter<<" "<<conflictedge<<" "<<bestConflictedge<<" "<<k<<endl;
    }
    }
